...
├── animation_knowledge_base/ # ★ body-part-level motion KB: actions/*.json = ONE store of 2454 records, schema
│                             #   motionkb/v4, 8 of them status:accepted (9-channel); raw/ = frozen per-frame
│                             #   sampling; frames/ = the eight-view render ring, 24 JPEG a clip (runtime
│                             #   retrieval input, lfs — the 8 accepted actions' are tracked, the 2446
│                             #   corpus clips' are gitignored like their dumps); derived/;
│                             #   schema/ = v1..v4 + CHANGELOG; engine_mask_map.json; manifest.json; README
...
>
> The agent side reaches the KB through one configured path (`MOTIONKB_DIR`), loads the accepted subset
> into memory at startup (8 records, 78 KB, selected through `manifest.json` rather than by opening all
> 2454), and treats it as read-only; the only writer is the offline pipeline.
...
All clips are Humanoid muscle clips retargeted to the single `nurse_avatar.fbx`, and all are in-place (`has_root_motion=false`). `typing` (added 2026-06-13) is the first **seated** action — it holds the whole body in a sitting pose and uses the two-handed `laptop` IK group. There is no stored `posture` field to read: the agent derives it from the measured `mean_body_height` (`kbindex.posture_of`), which is why the executor can refuse to walk a seated character off a chair without the KB ever having been told she is on one.

> **v4 note (the accepted store, as of 2026-08-26):** these same 8 actions are the **v4 9-channel** accepted records (`schema_version=motionkb/v4`, `status: accepted`) — the *action set* has not changed since v1. What changed along the way: v2 gave the body-part split its current shape (legs/hands left/right, a `root` channel) and moved extraction into Python; v3 replaced the scalar posture triple with the stored `mean_pose`; v4 deleted the composition-level fields entirely. The KINEMATIC numbers are in the records themselves (`agent/animation_knowledge_base/actions/<id>.json`, `channels.*.motion_magnitude` / `mean_pose`); the semantic half is `action_description` plus each channel's `motion_description`. Field semantics live in [ADR 0007](docs/adr/0007-v2-body-part-split.md) + [ADR 0021](docs/adr/0021-kinematic-facts-not-classifications.md) + [ADR 0022](docs/adr/0022-the-kb-describes-the-agent-decides.md) and the v4 schema; the retired v1 field reference is the appendix in `agent/animation_knowledge_base/README.md` (preserved at git tag `kb/v1`).
