...
│                             #   candidate/ = empty staging; v1 retired to git tag kb/v1; _raw/ = frozen per-frame
│                             #   sampling; _frames/ = render frames (runtime retrieval input, lfs); _reports/ = run-log;
│                             #   schema/ = v1+v2 + CHANGELOG; engine_mask_map.json; kb_manifest.json; README
└── README.md                 # what the KB is and why it is versioned here
```

> **Why the KB stays here while the pipeline left.** The KB cannot be regenerated without Unity — `_raw/`
> comes from in-engine `AnimationMode` sampling and `_frames/` from in-engine rendering — and it grows only
...
One `agent/kb/<action_id>.json` per animation. Full field documentation in `agent/kb/README.md`.
...
All clips are Humanoid muscle clips retargeted to the single `nurse_avatar.fbx`, and all are in-place (`has_root_motion=false`). `typing` (added 2026-06-13) is the first **seated** action — it locks the whole body into a sitting pose and uses the two-handed `laptop` IK group; see the `posture` field in `agent/kb/README.md`.

> **v2 note (the accepted store, as of 2026-06-24):** these same 8 actions are now the **v2 9-channel** root accepted store (`schema_version=motionkb/v2`, `status: accepted`) — the *action set* is unchanged; what changed vs the old v1 is the body-part split (legs/hands now left/right, a `root` channel, an orthogonal `ik_goals` layer) and the Python extractor. The "base/overlay" classification above carries over; the MEASURED magnitudes are in `agent/kb/_reports/extract_run.md`; the SEMANTIC per-channel 5-tuple is filled + verified (ADR 0008). The v2 field semantics live in [ADR 0007](docs/adr/0007-v2-body-part-split.md) + [ADR 0008](docs/adr/0008-vlm-proposed-authored-fields.md) + the v2 schema; the retired v1 field reference is the appendix in `agent/kb/README.md` (preserved at git tag `kb/v1`).
...
- [docs/adr/](docs/adr/) — architecture decision records (0001–0008); **[ADR 0007](docs/adr/0007-v2-body-part-split.md)** is the v2 9-channel split + the engine-decoupled Python extractor (supersedes ADR 0003); **[ADR 0008](docs/adr/0008-vlm-proposed-authored-fields.md)** is the VLM-propose + consistency-gate loop for the SEMANTIC fields.
- [docs/specs/motionkb-v2-spec.md](docs/specs/motionkb-v2-spec.md) — the v2 design narrative; `agent/kb/schema/CHANGELOG.md` records the v1 and v2 schema changes.
