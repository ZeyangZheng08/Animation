...
- **Body-part-level motion knowledge base `agent/animation_knowledge_base/`** — one JSON per nursing animation; the **canonical store** for the future Python RAG. The 8 nursing actions are `status: accepted` inside one store of 2454 records (the rest is the kinematic-only Mixamo corpus). Schema **`motionkb/v4`**: a **9-channel** split (`root` + `torso` + `head` + `left/right_arm` + `left/right_leg` + `left/right_hand`), produced by the engine-decoupled **Python extractor**. **A record answers two questions and no others** — what the action LOOKS like (`action_description`) and how each body part MOVES (the per-channel `motion_description` plus the **KINEMATIC** block: `state_label`, `motion_magnitude`, `raw_measurement`, `mean_pose`, and the root's `mean_body_height` / `mean_body_tilt_deg`). [**ADR 0022**](docs/adr/0022-the-kb-describes-the-agent-decides.md) (2026-08-26) deleted every field that stated a decision about a COMBINATION on a record describing one clip: `ik_goals`, the whole `composability` block, `mask_coverage`, `tags`, `display_name`, and the per-channel `role`/`motion_type`/`contact`/`constraint`/`target`. Composition, contact, IK and channel ownership are runtime decisions the agent makes with the task and the scene in hand. No number moved on that change (`metric_formula_version` stays v3.0.0) and the eight descriptions were carried through verbatim. The two description fields are VLM-proposed via the [ADR 0008](docs/adr/0008-vlm-proposed-authored-fields.md) loop (a VLM proposes from rendered frames; human `author` review is optional, so the current store is auto-accepted **`vlm_accepted`** with `verified_against_screenshots=false`), and the validator gates them for completeness rather than self-consistency: `python validate_motionkb.py`, run from the agent repo → **2454/2454**. **As of 2026-06-25 the proposing is now a program** — `render → propose → author` (`extract.py` + `vlm_openai.py` + `propose.py`, all in the agent repo since 2026-08-05), and all 8 actions were **re-proposed by `gpt-5.5-2026-04-23`** (which at the time also proposed the functional `action_id` from the frames — `nurse_give_meds → giving_pills`, `nurse_grab_aspirin → grab_bottle`); the MEASURE half is now keyed by clip name. The prior `claude-opus-4-8` proposal is preserved in `agent/motionkb_build/archive/authored_claude_backup/`; MEASURED was untouched (golden 8/8). **The prompt was rewritten for the corpus pass on 2026-08-27** and the describer now writes nine sentences and nothing else: one `action_description` plus one `motion_description` per anatomical channel, returned as nine labelled lines rather than JSON, with `state_label` as the only measurement in the prompt. Naming moved to acceptance — dozens of corpus clips are walk variants that would collide on one `action_id`, and a record keeps its `clip_name` key until a human accepts it under a name. No record changed: the prompt decides what the next 2446 descriptions are written from, not what the eight existing ones say.
  ...
  - **The 2446-clip Mixamo corpus is measured and rendered; its descriptions are not written yet.** Every corpus record carries the full KINEMATIC block ([ADR 0014](docs/adr/0014-corpus-enters-measured-only.md), 2026-08-21), and as of **2026-08-27** every corpus clip also has its eight-view evidence ring on disk — 57,680 JPEGs, 3.5 GB, rendered by `ingest_corpus.py render` with no failures. So the corpus is retrievable **by measurement** today (*"legs dynamic, torso static"* returns every walk in the library) and not yet by meaning. Writing the nine sentences each — `action_description` plus one `motion_description` per anatomical channel — is the next pass, planned as a local ~27B Qwen on HPC rather than 2446 hosted-API calls. Corpus `raw/mx_*` and `frames/mx_*` are gitignored as regenerable derivatives; the eight accepted actions' are tracked.
