...
**Retrieval-first.** Offline, Unity-compatible FBX motion assets are parsed into a structured representation on a unified skeleton, indexing both whole animation clips and body-part-level segments (per-part trajectories, key poses, root motion, phase information, and a description of how each body part moves). What the index deliberately does **not** store is any decision about a combination — what a hand holds, which parts a composition should draw from which clip, where an IK goal is pinned. Those depend on the task and the room, so they are decided at runtime by the agent ([ADR 0022](docs/adr/0022-the-kb-describes-the-agent-decides.md)). At runtime, given a natural-language intent and the current scene state, a **single LLM agent inside deterministic scaffolding** performs semantic understanding, action decomposition, retrieval selection, and symbolic assembly planning:
...
**Hard geometric gates at every stage** keep results executable in the real scene: intent (target reachability, orientation, scene compatibility) → retrieval (whether the retrieved part-segments can in fact be composed — channel coverage and contention — and transition validity) → assembly (acceleration discontinuity, foot skate, penetration, end-effector error) → scene landing (world-space geometry between the character and objects/ground/environment). On failure, the scaffolding feeds the reason back to the agent and rolls back to the corresponding upstream stage to replan.
...
- **Body-part-level motion knowledge base `agent/animation_knowledge_base/`** — one JSON per animation clip; the **canonical store** the runtime agent retrieves over. [**ADR 0023**](docs/adr/0023-the-general-library-is-the-knowledge-base.md) (2026-09-02) made it **2446 general-purpose Mixamo clips and nothing else** — all `status: accepted`, `action_id` equal to the clip name — and moved the eight hand-authored nursing actions out to `agent/nursing_assets/`, which nothing reads: an evaluation whose clips sit in the index it scores is not held out. Schema **`motionkb/v4`**: a **9-channel** split (`root` + `torso` + `head` + `left/right_arm` + `left/right_leg` + `left/right_hand`), produced by the engine-decoupled **Python extractor**. **A record answers two questions and no others** — what the action LOOKS like (`action_description`) and how each body part MOVES (the per-channel `motion_description` plus the **KINEMATIC** block: `state_label`, `motion_magnitude`, `raw_measurement`, `mean_pose`, and the root's `mean_body_height` / `mean_body_tilt_deg`). [**ADR 0022**](docs/adr/0022-the-kb-describes-the-agent-decides.md) (2026-08-26) deleted every field that stated a decision about a COMBINATION on a record describing one clip: `ik_goals`, the whole `composability` block, `mask_coverage`, `tags`, `display_name`, and the per-channel `role`/`motion_type`/`contact`/`constraint`/`target`. Composition, contact, IK and channel ownership are runtime decisions the agent makes with the task and the scene in hand. No number moved on that change (`metric_formula_version` stays v3.0.0) and the eight descriptions were carried through verbatim. The two description fields are VLM-proposed via the [ADR 0008](docs/adr/0008-vlm-proposed-authored-fields.md) loop (a VLM proposes from rendered frames; human `author` review is optional, so the current store is auto-accepted **`vlm_accepted`** with `verified_against_screenshots=false`), and the validator gates them for completeness rather than self-consistency: `python validate_motionkb.py`, run from the agent repo → **2446/2446**. **As of 2026-06-25 the proposing is now a program** — `render → propose → author` (`extract.py` + `vlm_openai.py` + `propose.py`, all in the agent repo since 2026-08-05), and the eight nursing actions were **re-proposed by `gpt-5.5-2026-04-23`** (which at the time also proposed the functional `action_id` from the frames — `nurse_give_meds → giving_pills`, `nurse_grab_aspirin → grab_bottle`); the MEASURE half is now keyed by clip name. The prior `claude-opus-4-8` proposal is preserved in `agent/motionkb_build/archive/authored_claude_backup/`; MEASURED was untouched (golden 8/8). **The prompt was rewritten for the corpus pass on 2026-08-27** and the describer now writes nine sentences and nothing else: one `action_description` plus one `motion_description` per anatomical channel, returned as nine labelled lines rather than JSON, with `state_label` as the only measurement in the prompt. Naming moved to acceptance — dozens of corpus clips are walk variants that would collide on one `action_id`, and a record keeps its `clip_name` key until a human accepts it under a name. That prompt then drove the corpus pass the same day — `qwen3.8-27b` on HPC wrote the descriptions into all 2446 corpus records; the eight existing ones were not touched.
  - **The former v1 store (6-part, schema `motionkb/v1`) was RETIRED by this promotion** but is preserved at git tag **`kb/v1`** for rollback (ADR 0005 / [docs/ROLLBACK.md](docs/ROLLBACK.md)); `agent/animation_knowledge_base/candidate/` was the staging area for future re-extractions until [ADR 0016](docs/adr/0016-one-store-status-is-the-membership-test.md) (2026-08-21) merged it into `agent/animation_knowledge_base/actions/` — one store where `status` is the membership test and acceptance renames a record rather than moving it; since [ADR 0023](docs/adr/0023-the-general-library-is-the-knowledge-base.md) every one of the 2446 records in it is accepted. Decision records: [ADR 0007](docs/adr/0007-v2-body-part-split.md) (supersedes ADR 0003), [ADR 0008](docs/adr/0008-vlm-proposed-authored-fields.md); design narrative: [docs/specs/motionkb-v2-spec.md](docs/specs/motionkb-v2-spec.md) (the v2 split, several contract versions stale); live contract: `agent/animation_knowledge_base/schema/motionkb.v4.schema.json`, with v1/v2/v3 kept on disk as the contracts of their snapshots. Rollback tag: `kb/v4`.
  - **The 2446-clip Mixamo corpus is measured, rendered and described.** Every corpus record carries the full KINEMATIC block ([ADR 0014](docs/adr/0014-corpus-enters-measured-only.md), 2026-08-21); as of **2026-08-27** every corpus clip also has its eight-view evidence ring on disk — 57,680 JPEGs, 3.5 GB, rendered by `ingest_corpus.py render` with no failures — and the **semantic pass ran the same day**: `qwen3.8-27b`, served locally on HPC rather than through 2446 hosted-API calls, read those rings and wrote the nine sentences into every record (`action_description` plus one `motion_description` per anatomical channel). So the corpus is now retrievable both **by measurement** (*"legs dynamic, torso static"* returns every walk in the library) and **by meaning**. The described records stayed `status: candidate` until [ADR 0023](docs/adr/0023-the-general-library-is-the-knowledge-base.md) (2026-09-02) accepted all 2446 of them under their own clip names — describing a clip is not accepting it, and neither pass moved a kinematic value. Corpus `raw/mx_*` and `frames/mx_*` are gitignored as regenerable derivatives.
- **Runtime executor `Assets/Scripts/AgentRuntime/`** — the engine half of Phase 2, 15 components. `AgentLink` holds the one WebSocket connection to the agent service (the agent is the *server*; the editor drops managed state on every recompile, so the reconnecting party must be this side) and speaks protocol v4, mirrored from the agent repo's `agent/protocol.py` — `Protocol.cs` is changed second, never first. `SceneRegistry`/`SceneQueryService` answer typed scene predicates (identity, category, coarse relations; coordinates only when explicitly asked for) and resolve the destinations that are not objects — `near:<object_id>`, and `view:left|right|ahead|behind` relative to the runtime camera, sampled onto the navmesh with the metres staying engine-side. `MotionComposer` + `ClipLibrary` play an assembled motion on a PlayableGraph of masked layers, to a timetable computed agent-side; **a layer may carry a fractional weight and its own entry phase, so one body channel can be driven by two clips at once** (measured, not assumed — see the ADR 0004 amendment), and **a layer may play only part of its clip** — an entry *and an exit* frame, looping inside that window where the motion repeats, so an overlay contributes one chest compression rather than thirty. Which frames those are is measured agent-side from the frozen pose dumps; nothing here chooses them. `PoseSynth` generates the frames between standing and seated **in either direction**, closed-loop on measured hip height. `Locomotion` walks her (every clip is in-place), and reports a remaining distance of -1 rather than an infinity a JSON serializer would turn into a string. `IkBinder` binds hands and gaze to scene objects on top of Animation Rigging, each binding engaging at the second its own step comes due. `GateProbe` measures the pose that actually played, in `LateUpdate` after IK, and reports metrics with no defensible threshold as *measured* rather than failing a plan on an invented cutoff — but it is no longer the only verdict, and since **v4 it is not the deciding one**. **A commit is played through on a hidden duplicate of the character first** (`ValidationCharacter`), at fixed timestep and with every renderer off, and only a pass reaches the visible one; the route is *previewed* rather than walked (`Locomotion.Preview`) so the motion is judged standing where the walk will actually leave her. Both paths judge through one `GateEvaluator` and are armed by one `GateArming`, so the check that runs before a plan plays and the one that runs while it plays cannot drift apart. `GateProbe`'s job is now to catch the scene changing under a plan that was already checked. See **[ADR 0009](docs/adr/0009-check-before-you-play.md)**. **No LLM and no generated C# on this side** — typed messages only; the MCP bridge that does ship C# is offline-only and builds the KB (§4).
...
├── animation_knowledge_base/ # ★ body-part-level motion KB: actions/*.json = ONE store of 2446 records, schema
│                             #   motionkb/v4, ALL status:accepted (9-channel), action_id = clip name;
│                             #   raw/ = frozen per-frame sampling; frames/ = the eight-view render ring,
│                             #   24 JPEG a clip (runtime retrieval input, lfs — gitignored like the dumps);
│                             #   derived/ = segments.json + posture.json (the posture sidecar the runtime
│                             #   refuses to start without); schema/ = v1..v4 + CHANGELOG;
│                             #   engine_mask_map.json; manifest.json; README
├── nursing_assets/           # ★ FROZEN, READ BY NOTHING: the 8 hand-authored nursing actions — records,
│                             #   dumps, frames, and the two derived tables that covered those eight.
│                             #   Held out for a nursing evaluation that does not exist yet (ADR 0023)
├── legacy/eval_8_actions/    #   the retired twelve-case retrieval eval set, archived beside its records
├── motionkb_build/           #   build-only artifacts: run reports, corpus enumeration, superseded-record archive
...
>
> The agent side reaches the KB through one configured path (`MOTIONKB_DIR`), loads the whole accepted
> store into memory at startup (2446 records in ~0.55 s, selected through `manifest.json` rather than by
> walking the directory), and treats it as read-only; the only writer is the offline pipeline. It also
> reads `derived/posture.json` and refuses to start without one that matches the current algorithm
> version and covers every record.
...
├── Scripts/AgentRuntime/     # ★ the runtime executor (15 components): AgentLink (WebSocket client) + Protocol, AgentCharacter, MotionComposer, ClipLibrary, PoseSynth, PostureTransitionEvaluator, IkBinder, Locomotion, SceneRegistry/SceneQueryService, GateEvaluator/GateArming/GateProbe, ValidationCharacter
...
- `docs/adr/` — architecture decision records (0001–0022). `docs/specs/` — the v2 design narrative.

**The agent repository** (`~/Research/animation_agent` on WSL) holds the engine-decoupled **Python** MotionKB **v4** pipeline: `config.py` channels/bone-map/divisors/thresholds · `metrics.py` formulas · `paths.py` KB location + write discipline · `extract.py` orchestration (`register|resolve-controller|emit-sampler|sample|assemble|render|propose|author|migrate`, the last being the v3→v4 converter) · `unity_sampler.py` the generic pose-sampler + frame renderer run over Unity MCP · `vlm_openai.py` / `vlm_anthropic.py` the two vision clients (one `describe` each, picked by `MOTIONKB_VLM`) + `propose.py` the prompt, the nine-line reply parser and the completeness loop · `ingest_corpus.py` the bulk path that measured and rendered the 2446-clip Mixamo corpus (`index|register|sample|measure|render|status`, the slow two resumable) · `build_posture.py` the posture sidecar (four coarse states per clip and how far each travels, [ADR 0024](docs/adr/0024-kinematic-posture-states-and-seat-alignment.md)) · `validate_motionkb.py` / `test_golden_extraction.py` / `gen_kb_manifest.py` / `build_posture.py --check` / `validate_guids.py` the five gates, run together by `check_kb.sh`. It also holds the **runtime service** that drives this project: `agent/` — retrieval over the KB, the deterministic channel partition and seam schedule, the ReAct loop, the model backends, and the typed contract `agent/protocol.py` that `Assets/Scripts/AgentRuntime/Protocol.cs` mirrors — with `cli.py` as the service and `terminal.py` as the console that opens on Play. It reads this repo's `agent/animation_knowledge_base/` through `MOTIONKB_DIR`. See its own `README.md`.
...
One `agent/animation_knowledge_base/actions/<action_id>.json` per animation. Full field documentation in `agent/animation_knowledge_base/README.md`.

| action_id | Controller state | Trigger | Carriage † |
|---|---|---|---|
| `idle` | Idle | default | standing (0.975) |
| `walking` | Walk_N | `Speed`>0.01 | standing (0.950) |
| `typing` | Typing | `typing` | **seated** (0.647) |
| `giving_pills` | Give Aspirin | `giveAspirin` | standing (0.941) |
| `cpr` | CPR | `CPR` | standing (1.058) |
| `grab_bottle` | Get Aspirin | `getAspirin` | standing (0.978) |
| `check_pulse` | Check Pulse | `pulse` | standing (0.928) |
| `bvm` | BVM | `BVM` | standing (0.859) |

> **† Carriage is MEASURED, not labelled.** It is the root channel's `mean_body_height` binned at **0.75**: `typing` reads 0.647 and the other seven fall between 0.859 (`bvm`, leaning over a patient) and 1.058, so 0.75 sits in the middle of a wide gap. `mean_body_tilt_deg` is deliberately not consulted — it measures forward lean, and `cpr` (44.6°) and `bvm` (39.3°) lean harder than the seated clip (8.7°). This column used to read "MotionKB role", the action's `composability.base_or_overlay`; that field was deleted by [ADR 0022](docs/adr/0022-the-kb-describes-the-agent-decides.md), because whether a clip is foundational or grafted on depends on what it is being composed with — `walking` is a base under a carry and an overlay under `cpr`. **This column is not the Unity Animator layer the clip lives on, and neither was the old one.** In the actual `NurseAnimator.controller`, _all_ of these action clips are states on the **Base Layer**, played **one at a time from Idle** (no AnyState; action → Idle → next action). The only _true_ Animator overlay layer is the **Upperbody Layer** (`Hold Pills` bool → carry a bottle while walking; masked to right arm + right fingers). Real body-part composition is done by the Phase-2 runtime executor from the agent's plan, not by this controller, which does runtime co-playback only, for disjoint parts.

All clips are Humanoid muscle clips retargeted to the single `nurse_avatar.fbx`, and all are in-place (`has_root_motion=false`). `typing` (added 2026-06-13) is the first **seated** action — it holds the whole body in a sitting pose and uses the two-handed `laptop` IK group. There is no stored `posture` field to read: the agent derives posture from the frozen dumps, since [ADR 0024](docs/adr/0024-kinematic-posture-states-and-seat-alignment.md) as a segmentation over time in `derived/posture.json` rather than as one label per clip, which is why the executor can refuse to walk a seated character off a chair without the KB ever having been told she is on one.

> **v4 note (as of 2026-09-02):** these same 8 actions are **v4 9-channel** records (`schema_version=motionkb/v4`), and since [ADR 0023](docs/adr/0023-the-general-library-is-the-knowledge-base.md) they live in `agent/nursing_assets/` rather than in the knowledge base — the accepted store is the 2446-clip Mixamo corpus. They are unchanged in every field — the *action set* has not changed since v1. What changed along the way: v2 gave the body-part split its current shape (legs/hands left/right, a `root` channel) and moved extraction into Python; v3 replaced the scalar posture triple with the stored `mean_pose`; v4 deleted the composition-level fields entirely. The KINEMATIC numbers are in the records themselves (`agent/nursing_assets/actions/<id>.json`, `channels.*.motion_magnitude` / `mean_pose`); the semantic half is `action_description` plus each channel's `motion_description`. Field semantics live in [ADR 0007](docs/adr/0007-v2-body-part-split.md) + [ADR 0021](docs/adr/0021-kinematic-facts-not-classifications.md) + [ADR 0022](docs/adr/0022-the-kb-describes-the-agent-decides.md) and the v4 schema; the retired v1 field reference is the appendix in `agent/animation_knowledge_base/README.md` (preserved at git tag `kb/v1`).
...
- [docs/adr/](docs/adr/) — architecture decision records (0001–0022); **[ADR 0022](docs/adr/0022-the-kb-describes-the-agent-decides.md)** is the current contract — the KB describes, the agent decides (supersedes ADR 0004 entirely and the SEMANTIC-5-tuple half of ADR 0008); **[ADR 0009](docs/adr/0009-check-before-you-play.md)** is the pre-execution check and protocol v4; **[ADR 0007](docs/adr/0007-v2-body-part-split.md)** is the 9-channel split + the engine-decoupled Python extractor (supersedes ADR 0003); **[ADR 0021](docs/adr/0021-kinematic-facts-not-classifications.md)** is the stored `mean_pose`.
- [docs/specs/motionkb-v2-spec.md](docs/specs/motionkb-v2-spec.md) — the v2 design narrative, kept as such and several contract versions stale; `agent/animation_knowledge_base/schema/CHANGELOG.md` records every schema change from v1 to v4.
