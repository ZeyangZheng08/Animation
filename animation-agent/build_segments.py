...
live from `raw`, deleting the file costs a rebuild and nothing else, and nothing in the motionkb/v3
...
def main(argv):
    ...
    print("%-13s %-11s %-5s %9s %8s %9s %8s %7s"
          % ("action", "channel", "role", "travel", "peak/s", "window", "cycle", "keeps"))

    cyclic, trimmed = [], []
    for action_id in sorted(table):
        channels = kb.channels(action_id)
        for seg in table[action_id]:
            role = (channels.get(seg["channel"]) or {}).get("role", "free")
            keeps = seg["frames"] / float(seg["clip_frames"])
            cycle = ("%df @%.2f" % (seg["cycle_frames"], seg["cycle_residual_deg"])
                     if seg["cycle_frames"] else "-")
            print("%-13s %-11s %-5s %9.1f %8.1f %9s %8s %6.0f%%"
                  % (action_id, seg["channel"], role[:4], seg["travel_deg"], seg["peak_deg_per_s"],
